...
function showCards(){
    let returnString= "";
        player.forEach((Elisabeta) => {
                    returnString += Elisabeta.value + " ";
                  });
...
function compareScore() {
    ...
    if(compScore>21){
      alert("Computer lost: " + compScore);
    }
      else if(compScore>playerScore){
        alert("Computer won! You lost. More luck next time! Computer score was " + compScore + " and yours was " + playerScore);
      }
      else{
        if(playerScore>21){
          alert("You lost. Your score is: " + playerScore);
        }
          else {
...
//the player needs to die if >21
//the comp needs to die if > 21
// reset button - reset the game
//stop when a condition is checked 
